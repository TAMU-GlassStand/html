...
channel3 = 13	#measures push button interrupt
channel4 = 15	#measures arduino gpio output
...
gpio.setup(channel3, gpio.IN, pull_up_down = gpio.PUD_UP)	#gpio input for measuring push button
gpio.setup(channel4, gpio.IN, pull_up_down = gpio.PUD_DOWN)	#gpio input for measuring arduino gpio output
...
def bright_up():
	...
	if user_location==0:
		# Brightness Plus
		m.move(x_pos*44, y_pos*42.5)
		time.sleep(0.05)
		m.click(x_pos*44, y_pos*42.5, 1)
		time.sleep(0.05)
		
		# Apply Changes  
		m.move(x_pos*38, y_pos*44.5)
		time.sleep(0.05)
		m.click(x_pos*38, y_pos*44.5, 1)
		time.sleep(0.05)
...
def bright_down():
	...
	if user_location==0:
		# Brightness Minus
		m.move(x_pos*51.5, y_pos*42.5)
		time.sleep(0.05)
		m.click(x_pos*51.5, y_pos*42.5, 1)
		time.sleep(0.05)
...
def page_back():
	...
	if user_location==2 and enter_count>0:
		k.press_key(k.alt_key)
...
def enter_button():
	...
	return;
#----------------------------------------------------------------------------------
#define night mode button (Invert image color)
def night_mode():
	x_dim, y_dim=m.screen_size()
	x_pos=x_dim/100
	y_pos=y_dim/100
...
def auto_shutdown(pause=delay, gpio_channel=channel1):
	debug_output.write("goodbye\n")
	k.press_key(k.control_key)
	k.tap_key('w', 2)
	k.release_key(k.control_key)
...
def autobrightness(delay, ambientinput_old, user_brightness_old, evm_address, ambient_address, rw_reg, resp_reg):
	debug_output.write("beginning of autobrightness function\n")
	...
	if ambientinput>=ambientinput_old*(1+delta) or ambientinput<ambientinput_old*(1-delta) or user_brightness!=user_brightness_old:
		...
		adjustment = 0.25*math.log10( (ambientinput+1)/(amb_thresh+1))
		if adjustment > 0.25:
			adjustment = 0.25
	...
	debug_output.write("end of autobrightness function\n")
...
try:
	...
	#add gpio event detector interrupt for push button
	gpio.add_event_detect(channel3, gpio.FALLING, callback = auto_shutdown, bouncetime=100)
	
	#add gpio event detector interrupt for arduino
	gpio.add_event_detect(channel4, gpio.RISING, callback = auto_shutdown, bouncetime=100)
